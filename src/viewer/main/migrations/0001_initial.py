# -*- coding: utf-8 -*-
import datetime
from south.db import db
from south.v2 import SchemaMigration
from django.db import models


class Migration(SchemaMigration):

    def forwards(self, orm):
        # Adding model 'Project'
        db.create_table('main_project', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('key', self.gf('django.db.models.fields.CharField')(unique=True, max_length=255)),
            ('name', self.gf('django.db.models.fields.CharField')(max_length=300)),
        ))
        db.send_create_signal('main', ['Project'])

        # Adding model 'YearlyPlan'
        db.create_table('main_yearlyplan', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('project', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['main.Project'])),
            ('hours', self.gf('django.db.models.fields.FloatField')()),
            ('created_at', self.gf('django.db.models.fields.DateField')()),
            ('active', self.gf('django.db.models.fields.BooleanField')(default=True)),
        ))
        db.send_create_signal('main', ['YearlyPlan'])

        # Adding model 'MonthlyPlan'
        db.create_table('main_monthlyplan', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('yearlyplan', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['main.YearlyPlan'])),
            ('hours', self.gf('django.db.models.fields.FloatField')()),
            ('starts', self.gf('django.db.models.fields.DateField')()),
            ('finished', self.gf('django.db.models.fields.DateField')(null=True, blank=True)),
        ))
        db.send_create_signal('main', ['MonthlyPlan'])

        # Adding model 'HoursAdd'
        db.create_table('main_hoursadd', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('monthlyPlan', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['main.MonthlyPlan'])),
            ('hours', self.gf('django.db.models.fields.FloatField')(null=True, blank=True)),
            ('period', self.gf('django.db.models.fields.DateField')()),
        ))
        db.send_create_signal('main', ['HoursAdd'])


    def backwards(self, orm):
        # Deleting model 'Project'
        db.delete_table('main_project')

        # Deleting model 'YearlyPlan'
        db.delete_table('main_yearlyplan')

        # Deleting model 'MonthlyPlan'
        db.delete_table('main_monthlyplan')

        # Deleting model 'HoursAdd'
        db.delete_table('main_hoursadd')


    models = {
        'main.hoursadd': {
            'Meta': {'object_name': 'HoursAdd'},
            'hours': ('django.db.models.fields.FloatField', [], {'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'monthlyPlan': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['main.MonthlyPlan']"}),
            'period': ('django.db.models.fields.DateField', [], {})
        },
        'main.monthlyplan': {
            'Meta': {'object_name': 'MonthlyPlan'},
            'finished': ('django.db.models.fields.DateField', [], {'null': 'True', 'blank': 'True'}),
            'hours': ('django.db.models.fields.FloatField', [], {}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'starts': ('django.db.models.fields.DateField', [], {}),
            'yearlyplan': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['main.YearlyPlan']"})
        },
        'main.project': {
            'Meta': {'object_name': 'Project'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '255'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '300'})
        },
        'main.yearlyplan': {
            'Meta': {'object_name': 'YearlyPlan'},
            'active': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
            'created_at': ('django.db.models.fields.DateField', [], {}),
            'hours': ('django.db.models.fields.FloatField', [], {}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['main.Project']"})
        }
    }

    complete_apps = ['main']